...
class createSet:
    ...
    def runRandomGame(self):
        ...
        def recordResult(winner):
            ...
            # P1 win then 1, P2 win then 0
            self.__pValue.append(1 if winner == 1 else 0)
            self.__gameResult.append(winner)
            ...
            for j in range(len(p2)):
                ...
                self.__allP1.append(p1)
                self.__allP2.append(p2)
                # 2nd Player no possibility to win
                if (j == 0):
                    self.__pValue.append(0)
                else:
                    pValueCalculate()
                self.__gameResult.append(checkWin())
                
                if (j != (len(p2) - 1)) or (winner != 2):
                    if winner == 1:
                        singleGameStates[(p2[len(p2) - j - 1]) - 1] = 0
                    else:
                        singleGameStates[(p1[len(p1) - j - 1]) - 1] = 0
        ...
        return self.__allPossibleStates, self.__pValue, self.__allP1, self.__allP2, self.__gameResult
