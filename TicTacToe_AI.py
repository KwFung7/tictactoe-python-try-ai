...
def gameStart():
    ...
    # Load AI model
    AI_model = load_model('Player1_Model_Binary.h5')
    ...
    buRestart.config(command=lambda: (gameStart(), aiTurn()))
...
def modelPrediction():
    ...
    secondChoice_aiPrediction = 0
    aiPrediction = 0 
    best_position = 0
    secondChoice_position = 0
    
    # Initial state
    if (1 not in singleGameStep) and (-1 not in singleGameStep):
        
        for i in range(9):
            
            singleGameStep[0, i] = 1
            aiPrediction = AI_model.predict(singleGameStep)
            # Find best or second choice
            if aiPrediction > max_aiPrediction:
                secondChoice_aiPrediction = max_aiPrediction
                max_aiPrediction = aiPrediction
                secondChoice_position = best_position
                best_position = i
            singleGameStep[0, i] = 0
            
        return (max_aiPrediction, best_position) if rd.randrange(10) > 3 else (secondChoice_aiPrediction, secondChoice_position)
       
    # At least one step state
    else: 
